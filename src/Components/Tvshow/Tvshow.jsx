...
export default function Tvshow() {
    ...
    async function getTvs(page) {
      let { data } = await axios.get(`https://api.themoviedb.org/3/discover/tv?api_key=eac04f79494b94483227629a5ceeb89c&language=en-US&sort_by=popularity.desc&include_adult=false&include_video=true&page=${page}`)
      setTv(data?.results)
...
<Helmet>
    ...
    {tv [0] ? 
     ...
     {tv.map((item, index) => <div key={index} className='col-md-3 py-2'>
    ...
    <div className='loading d-flex vh-100 align-items-center justify-content-center'> <i className='fas fa-spinner fa-spin fa-8x'></i> </div>
      ...
      <nav className='py-5'>
        <ul className='pagination pagination-sm d-flex justify-content-center'>
         {nums.map((page,index)=><li key={index} onClick={()=>getTvs(page)} className='page-item p-1'>
